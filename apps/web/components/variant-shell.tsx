...
export function VariantShell({ projection, events, restoreStatus, saveState, online, latestSave, storageIssue, onRetrySave, dispatch }: { projection: PlayerProjection; events: GameEvent[]; restoreStatus?: RestoreStatus; saveState: "idle" | "saving" | "saved" | "error"; online: boolean; latestSave?: SaveEnvelope; storageIssue?: string; onRetrySave: () => void; dispatch: (command: GameCommand) => void }) {
  ...
  const [mobileProofFocus, setMobileProofFocus] = useState<"board" | "chain">("board");
  ...
  const boardReadiness = projection.reasoningBoards.map((board) => ({
    id: board.id,
    ready: board.slots.every((slot) => Boolean(slot.itemId)) && (board.slots.length <= 1 || board.connections.length >= board.slots.length - 1),
  }));
  const readyBoardCount = boardReadiness.filter((item) => item.ready).length;
  const activeBoardIndex = Math.max(0, projection.reasoningBoards.findIndex((board) => board.id === activeBoardId));
  const nextIncompleteBoard = projection.reasoningBoards.find((board) => board.id !== activeBoardId && !boardReadiness.find((item) => item.id === board.id)?.ready);
    ...
    <main id="main-content" tabIndex={-1} className={styles.variant} data-layout={layout} data-experience-stage={experienceStage} data-complex-proof={projection.reasoningBoards.length > 1 || undefined} data-golden-cadence={golden?.cadence} data-replay-tone={golden?.replayTone} data-evidence-mode={projection.case.presentation.evidenceVisualMode} data-high-contrast={highContrast || undefined} data-reduced-motion={reducedMotion || undefined} style={{ "--accent": projection.case.presentation.accent } as CSSProperties}>
      ...
      <div className={styles.board} inert={settingsOpen || undefined}>
        ...
        <section id="variant-questions" className={`${styles.middle} ${panel === "questions" ? styles.mobileVisible : ""}`}>
          ...
          <MomentumRail projection={projection} events={events} openingMoves={golden?.openingMoves} onNavigate={setPanel} />
        ...
        {revealTheory && <section id="variant-theory" className={`${styles.right} ${panel === "theory" ? styles.mobileVisible : ""}`}>
          ...
          {active && <>
            ...
            {projection.reasoningBoards.length > 1 && <div className={styles.mobileTaskBar} data-mobile-proof-controls>
              <small>BOARD {activeBoardIndex + 1}/{projection.reasoningBoards.length} · {readyBoardCount} 个结构已填满</small>
              <label>当前任务<select aria-label="切换当前推理板" value={activeBoardId} onChange={(event) => { setActiveBoardId(event.target.value); setCollapsedBoardIds((current) => current.filter((id) => id !== event.target.value)); setMobileProofFocus("board"); }}>{projection.reasoningBoards.map((board) => { const ready = boardReadiness.find((item) => item.id === board.id)?.ready; return <option key={board.id} value={board.id} data-board-mode={board.mode}>{ready ? "✓ " : "○ "}{board.title}</option>; })}</select></label>
              <div className={styles.proofFocus} aria-label="切换证明工作区"><button type="button" aria-pressed={mobileProofFocus === "board"} data-active={mobileProofFocus === "board" || undefined} onClick={() => setMobileProofFocus("board")}>当前板</button><button type="button" aria-pressed={mobileProofFocus === "chain"} data-active={mobileProofFocus === "chain" || undefined} onClick={() => setMobileProofFocus("chain")}>事件链</button></div>
              <span data-proof-gap>{lastProofGap ? `缺口：${PROOF_GAP_LABELS[lastProofGap]}` : activeBoardReady ? "当前结构已填满，可继续下一板" : "先完成当前板，不必同时展开全部任务"}</span>
              <div className={styles.mobileTaskActions}>{nextIncompleteBoard && <button type="button" onClick={() => { setActiveBoardId(nextIncompleteBoard.id); setCollapsedBoardIds((current) => current.filter((id) => id !== nextIncompleteBoard.id)); setMobileProofFocus("board"); }}>下一未完成板</button>}<button disabled={!projection.canSubmit || projection.solved} onClick={() => dispatch({ type: "submit_theory", theoryId: active.id })}>统一提交</button></div>
            </div>}
            ...
            <div data-mobile-proof-surface="board" data-mobile-active={mobileProofFocus === "board" || undefined}>
              {activeBoard && (activeBoardCollapsed ? <div className={styles.boardSummary} data-board-collapsed><span><small>{activeBoard.mode}</small><b>{activeBoard.title}</b><em>{activeBoardPlaced}/{activeBoard.slots.length} 槽位 · {activeBoard.connections.length} 条关系</em></span><button onClick={() => setCollapsedBoardIds((current) => current.filter((id) => id !== activeBoard.id))}>重新展开</button></div> : <><div className={styles.boardControl}><span>{activeBoardReady ? "当前板结构已填满；统一提交仍会验证关系是否正确" : "一次只完成当前推理任务"}</span>{activeBoardReady && (nextIncompleteBoard ? <button onClick={() => { setCollapsedBoardIds((current) => [...new Set([...current, activeBoard.id])]); setActiveBoardId(nextIncompleteBoard.id); }}>收起并前往下一板</button> : <button onClick={() => setCollapsedBoardIds((current) => [...new Set([...current, activeBoard.id])])}>收起当前板</button>)}</div><ReasoningBoard board={activeBoard} dispatch={dispatch} /></>)}
            </div>
            <div data-mobile-proof-surface="chain" data-mobile-active={mobileProofFocus === "chain" || undefined}>
