...
import { challengeRotation, emptyMasteryRecord, recordMasterySolve, type EvidencePlayerState, type GameCommand, type GameEvent, type PlayerProjection, type SaveEnvelope, type CaseMasteryRecord } from "@turtle-soup/mystery-core";
...
const EVIDENCE_STATUS: Record<EvidencePlayerState, string> = { available: "待检查", discovered: "新证据", examined: "已检查", connected: "已关联", verified: "已核实", dismissed: "已搁置" };
...
export function VariantShell({ projection, events, restoreStatus, saveState, online, latestSave, storageIssue, onRetrySave, dispatch }: { projection: PlayerProjection; events: GameEvent[]; restoreStatus?: RestoreStatus; saveState: "idle" | "saving" | "saved" | "error"; online: boolean; latestSave?: SaveEnvelope; storageIssue?: string; onRetrySave: () => void; dispatch: (command: GameCommand) => void }) {
  ...
  const profile = projection.reasoningBoards.length > 0 ? {
  ...
  return (
    <main id="main-content" tabIndex={-1} className={styles.variant} data-layout={layout} data-evidence-mode={projection.case.presentation.evidenceVisualMode} data-high-contrast={highContrast || undefined} data-reduced-motion={reducedMotion || undefined} style={{ "--accent": projection.case.presentation.accent } as CSSProperties}>
      ...
      <nav className={styles.mobileTabs} aria-label="调查区域" inert={settingsOpen || undefined}>{navLabels.map((label, index) => { const target = (["scene", "questions", "theory"] as const)[index]; return <button key={label} aria-controls={`variant-${target}`} aria-pressed={panel === target} data-active={panel === target || undefined} onClick={() => setPanel(target)}>{label}</button>; })}</nav>
      <div className={styles.board} inert={settingsOpen || undefined}>
        <section id="variant-scene" className={`${styles.left} ${panel === "scene" ? styles.mobileVisible : ""}`}>
          ...
          <div className={styles.evidenceRail}>
            {projection.evidence.map((evidence, evidenceIndex) => {
              const linked = active?.evidenceIds.includes(evidence.id) ?? false;
              const inspected = evidence.state !== "available" && evidence.state !== "discovered";
              return <article key={evidence.id} data-state={evidence.state}>
                <div className={styles.evidenceType}><span>{evidence.sourceLabel}</span><b>{EVIDENCE_STATUS[evidence.state]}</b></div>
                {inspected && <img className={styles.evidenceArt} src={evidence.visualAsset ?? `/assets/cases/${caseCode}/evidence-${String((evidenceIndex % 5) + 1).padStart(2, "0")}.webp`} alt={`${evidence.title}的档案局部图`} width={256} height={160} loading="lazy" />}
                <h3>{evidence.title}</h3><p>{evidence.observation}</p>
                <div className={styles.evidenceActions}>{evidence.state === "available" || evidence.state === "discovered" ? <button onClick={() => dispatch({ type: "set_evidence_state", evidenceId: evidence.id, state: "examined" })}>检查证据</button> : <><button data-active={linked || undefined} onClick={() => dispatch({ type: "link_theory_evidence", theoryId: projection.activeTheoryId, evidenceId: evidence.id, linked: !linked })}>{linked ? "移出证明" : "接入证明"}</button><button onClick={() => dispatch({ type: "set_evidence_state", evidenceId: evidence.id, state: evidence.state === "verified" ? (linked ? "connected" : "examined") : "verified" })}>{evidence.state === "verified" ? "取消核实" : "标记核实"}</button></>}</div>
              </article>;
            })}
          </div>
        </section>
        <section id="variant-questions" className={`${styles.middle} ${panel === "questions" ? styles.mobileVisible : ""}`}>
          ...
          <div className={styles.transcript} aria-live="polite">{projection.transcript.length === 0 ? <div className={styles.firstRun}><p>问题不是索要答案，而是一次有方向的验证。</p><ol><li><b>01</b>观察异常与来源</li><li><b>02</b>提问排除错误路径</li><li><b>03</b>把事件和证据接入证明链</li></ol></div> : projection.transcript.map((entry) => <article key={entry.id}><small>YOU · {entry.repeated ? "REPEATED" : "QUERY"}</small><p>{entry.rawQuestion}</p><div data-code={entry.answerCode}><b>{ANSWERS[entry.answerCode]}</b><span>{entry.answerText}{host.rewrites[entry.id] && <em className={styles.hostRewrite}><i>本地改写主持</i>{host.rewrites[entry.id].text}</em>}</span></div></article>)}</div>
          ...
          <div className={styles.prompts}>{projection.questionScaffolds.slice(0, 4).map((candidate) => <button key={candidate.queryId} onClick={() => dispatch({ type: "ask_text", rawText: candidate.label })}>{candidate.label}</button>)}</div>
        ...
        <section id="variant-theory" className={`${styles.right} ${panel === "theory" ? styles.mobileVisible : ""}`}>
          ...
          {active && <>
            ...
            <div className={styles.chain}><header><span>{profile.chain}</span><small>{active.evidenceIds.length} linked</small></header>{active.eventIds.length === 0 ? <p className={styles.empty}>把已知事件放进这条证明链。</p> : <ol>{active.eventIds.map((id, index) => { const item = eventMap.get(id); return item ? <li key={id}><i>{index + 1}</i><time>{item.timeLabel}</time><b>{item.label}</b><nav><button disabled={index === 0} onClick={() => dispatch({ type: "move_theory_event", theoryId: active.id, eventId: id, direction: -1 })} aria-label="事件上移">↑</button><button disabled={index === active.eventIds.length - 1} onClick={() => dispatch({ type: "move_theory_event", theoryId: active.id, eventId: id, direction: 1 })} aria-label="事件下移">↓</button><button onClick={() => dispatch({ type: "remove_theory_event", theoryId: active.id, eventId: id })} aria-label="移除事件">×</button></nav></li> : null; })}</ol>}</div>
            <div className={styles.eventBank}>{projection.eventOptions.map((item) => <button key={item.id} disabled={active.eventIds.includes(item.id)} onClick={() => dispatch({ type: "upsert_theory_event", theoryId: active.id, eventId: item.id })}><time>{item.timeLabel}</time>{item.label}</button>)}</div>
