...
export function HostRewriteControls({ settings, status, message, onChange, onClear }: { settings: HostRewriteSettings; status: "off" | "idle" | "working" | "fallback"; message: string; onChange: (settings: HostRewriteSettings) => void; onClear: () => void }) {
  ...
  return (
    <section className={styles.panel} aria-label="本地主持改写">
      ...
      <label className={styles.toggle}><span><b>启用本地主持</b><small>默认关闭；不参与答案与胜负</small></span><input type="checkbox" checked={settings.enabled} onChange={(event) => update("enabled", event.target.checked)} /></label>
      <label><span>localhost 端点</span><input type="url" value={settings.endpoint} disabled={!settings.enabled} onChange={(event) => update("endpoint", event.target.value)} spellCheck={false} /></label>
      <label><span>模型名</span><input value={settings.model} disabled={!settings.enabled} onChange={(event) => update("model", event.target.value)} spellCheck={false} /></label>
      <label><span>主持语气</span><select value={settings.style} disabled={!settings.enabled} onChange={(event) => update("style", event.target.value as HostRewriteStyle)}><option>冷静</option><option>低语</option><option>档案</option><option>紧张</option></select></label>
