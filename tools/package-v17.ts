...
const report = {
  ...
  artifact: { directory: "dist/v1.7-internal-rc-web-pwa", bytes: contentFiles.reduce((sum, path) => sum + statSync(path).size, 0), files: contentFiles.length, sha256: digest.digest("hex"), archive: { path: "dist/turtle-soup-v1.7-internal-rc-web-pwa.zip", bytes: zip.length, sha256: createHash("sha256").update(zip).digest("hex") } },
  preservedArtifacts,
  passed: zip.length <= 8_000_000 && preservedArtifacts.length === 7 && preservedArtifacts.every((item) => item.unchanged),
};
writeFileSync(resolve(root, "docs/v1.7-release-artifacts.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
