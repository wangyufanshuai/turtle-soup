...
if (!sw.includes("black-soup-v17-golden-path-rc-1")) failures.push("v1.7 cache identity missing");
const precache = JSON.parse(readFileSync(resolve(root, "docs/v1.7-precache.json"), "utf8")) as { passed?: boolean; entryCount?: number; totalBytes?: number };
...
const report = { reportVersion: "1.7", releaseProfile: "v1.7-internal-rc", generatedAt: new Date().toISOString(), status: "internal-rc / human-evaluation-pending", humanParticipants: 0, budgets, actual, cacheIdentity: "black-soup-v17-golden-path-rc-1", failures, passed: failures.length === 0 };
writeFileSync(resolve(root, "docs/v1.7-static-performance.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
