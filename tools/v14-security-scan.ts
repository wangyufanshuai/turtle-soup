...
const remoteReferences = texts.filter(({ text }) => /api\.openai\.com|google-analytics|segment\.io|mixpanel|sentry\.io/i.test(text)).length;
const diagnosticsLeak = texts.filter(({ text }) => /rawQuestion|commandLog|canonicalHypothesisId|solutionCertificate|hiddenFact/i.test(text)).length;
const failures = [
  ...
  diagnosticsLeak > truthBearingTextFiles ? `diagnostic or public bundle leak detected in ${diagnosticsLeak} files` : "",
].filter(Boolean);
const report = { reportVersion: "1.4", releaseProfile: "v1.4-internal-rc", generatedAt: new Date().toISOString(), humanParticipants: 0, hiddenIdentifierCount: hidden.length, targets: [{ name: "static-output", files: texts.length, truthBearingTextFiles, sourceMaps, remoteReferences, diagnosticsLeak, failures, passed: failures.length === 0 }], passed: failures.length === 0, qualification: "Static scans cover publication leakage and remote dependency boundaries; they do not prove resistance to active reverse engineering." };
writeFileSync(resolve(root, "docs/v1.4-security-scan.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
