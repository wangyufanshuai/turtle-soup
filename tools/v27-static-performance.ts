...
const actual = { staticBytes: all.reduce((sum, item) => sum + item.bytes, 0), initialJavaScriptBytes, maximumFileBytes: Math.max(0, ...all.map((item) => item.bytes)), fileCount: all.length, routeCount: all.filter((item) => /^case\/[^/]+\/index\.html$/u.test(item.relative)).length, archiveBytes: existsSync(resolve(root, "dist/turtle-soup-v2.7-internal-rc-web-pwa.zip")) ? statSync(resolve(root, "dist/turtle-soup-v2.7-internal-rc-web-pwa.zip")).size : Number.POSITIVE_INFINITY };
