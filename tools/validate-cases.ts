...
const reports = loaded.map(({ entry, caseFile, corpus }) => {
  ...
  const report = analyzeCaseQuality(caseFile, corpus);
    if (entry.seasonId === "season-2" || entry.seasonId === "season-4" || entry.seasonId === "season-5") {
    ...
    if (entry.seasonId === "season-5") {
      const count = Number(caseFile.id.match(/^c(\d+)/)?.[1] ?? 0);
      if (caseFile.events.length < 10 || caseFile.events.length > 15) report.errors.push({ severity: "error", code: "S5_EVENT_COUNT", subjectId: caseFile.id, message: `第五季事件数 ${caseFile.events.length} 不在 10–15 范围。` });
      if (caseFile.facts.length < 16 || caseFile.facts.length > 24) report.errors.push({ severity: "error", code: "S5_FACT_COUNT", subjectId: caseFile.id, message: `第五季事实数 ${caseFile.facts.length} 不在 16–24 范围。` });
      if (caseFile.evidenceItems.length < 10 || caseFile.evidenceItems.length > 15) report.errors.push({ severity: "error", code: "S5_EVIDENCE_COUNT", subjectId: caseFile.id, message: `第五季证据数 ${caseFile.evidenceItems.length} 不在 10–15 范围。` });
      if (caseFile.proofReplay.length < 6 || (count === 84 && (caseFile.chapters?.length ?? 0) < 2)) report.errors.push({ severity: "error", code: "S5_REPLAY_OR_CHAPTER", subjectId: caseFile.id, message: "第五季回放或 C84 章节门槛未满足。" });
      if (report.metrics.duplicateQuestionRate >= 10 || report.metrics.ambiguityRate < 2) report.errors.push({ severity: "error", code: "S5_LANGUAGE_GATE", subjectId: caseFile.id, message: "第五季语言覆盖或安全歧义门槛未满足。" });
      if ((caseFile.reasoningBoards?.length ?? 0) < 2 || (caseFile.solutionCertificate.minimumProofSets?.length ?? 0) < 2) report.errors.push({ severity: "error", code: "S5_BOARD_OR_PROOF", subjectId: caseFile.id, message: "第五季案件必须定义至少两个推理板和两组最小证明。" });
