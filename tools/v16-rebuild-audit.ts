...
const env = { ...process.env, TURTLE_SOUP_RELEASE_PROFILE: "v1.6-internal-rc" };
...
function treeDigest(dir: string): { sha256: string; files: number; bytes: number } {
  ...
  for (const path of paths) {
    ...
    bytes += data.length;
    digest.update(relative(dir, path).split(sep).join("/"));
    digest.update(data);
  }
...
function buildWindowsShell() {
  execSync("npm run build", { cwd: root, env, stdio: "inherit", shell: true });
}
function packageDirectNode() {
  execFileSync(process.execPath, ["--experimental-strip-types", "--experimental-transform-types", resolve(root, "tools/package-v16.ts"), root], { cwd: root, env, stdio: "inherit" });
}
...
const artifact = JSON.parse(readFileSync(resolve(root, "docs/v1.6-release-artifacts.json"), "utf8")) as { preservedArtifacts: Array<{ unchanged: boolean; version: string; expected: string; actual: string | null }> };
const checks = {
  ...
  historicalArtifactsPreserved: artifact.preservedArtifacts.length === 6 && artifact.preservedArtifacts.every((item) => item.unchanged),
};
const report = {
  reportVersion: "1.6",
  ...
  humanParticipants: 0,
  runs: [
    { id: "build-package-1", staticTree: firstTree, archive: firstArchive },
    { id: "build-package-2", staticTree: secondTree, archive: secondArchive },
  ],
...
writeFileSync(resolve(root, "docs/v1.6-rebuild-determinism.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
