...
const languageRecoveryFlow = v29 || v210 || v211;
...
try {
  ...
  for (const viewport of [{ name: "desktop", width: 1440, height: 900, mobile: false }, { name: "mobile", width: 390, height: 844, mobile: true }]) {
    ...
    if (languageRecoveryFlow) directRecoveryEntry = await dialog.getByRole("tab", { name: /^AI/ }).getAttribute("aria-selected") === "true";
    if (settingsTabCount > 0 && !languageRecoveryFlow) await dialog.getByRole("tab", { name: /^AI/ }).click();
    ...
    await page.waitForFunction((expected) => document.activeElement?.textContent?.trim() === expected, languageRecoveryFlow ? "配置 AI 问题理解" : "设置", { timeout: 2_000 }).catch(() => undefined);
    const focusReturned = await (languageRecoveryFlow ? recoveryTrigger : settingsButton).evaluate((node) => document.activeElement === node);
    ...
    const progressiveSettings = v28 || languageRecoveryFlow ? settingsTabCount === 3 && (languageRecoveryFlow ? directRecoveryEntry : experienceSelected) && dataToolsVisible : true;
    const recoveryScreenshotValid = !languageRecoveryFlow || Boolean(recoveryScreenshot && recoveryScreenshot.bytes > 5_000);
