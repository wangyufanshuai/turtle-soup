...
const entries = loadReleaseContent(root, "v1.8-internal-rc").entries;
...
const reportFiles = ["docs/v1.8-momentum-proxy.json", "docs/v1.8-browser-matrix.json"].filter((path) => existsSync(resolve(root, path)));
...
const momentumSource = readFileSync(resolve(root, "apps/web/lib/investigation-momentum.ts"), "utf8");
...
const boundaryChecks = {
  ...
  momentumAcceptsPublicProjectionOnly: momentumSource.includes("type PublicProjection = Pick<PlayerProjection") && !momentumSource.includes("CaseFile") && !momentumSource.includes("SolutionCertificate"),
...
const failures = [
  ...(!boundaryChecks.truthChunkExists ? ["no truth-bearing worker chunk"] : []), ...(sourceMaps ? [`${sourceMaps} source maps shipped`] : []),
  ...(remoteReferences.length ? [`${remoteReferences.length} remote references found`] : []), ...(publicHtmlLeaks.length ? [`${publicHtmlLeaks.length} public HTML truth leaks`] : []),
  ...(reportLeakTokens.length ? [`quality reports contain forbidden schema names: ${reportLeakTokens.join(", ")}`] : []), ...(!boundaryChecks.momentumAcceptsPublicProjectionOnly ? ["momentum proxy boundary unsafe"] : []),
  ...(!boundaryChecks.workerOwnsCaseLoading ? ["worker case-loading boundary not found"] : []), ...(!boundaryChecks.localHostRewriteOnly ? ["local host rewrite restriction not found"] : []),
];
const report = { reportVersion: "1.8", releaseProfile: "v1.8-internal-rc", generatedAt: new Date().toISOString(), status: "internal-rc / human-evaluation-pending", humanParticipants: 0, hiddenIdentifierCount: hidden.length, targets: [{ name: "static-output", files: texts.length, truthBearingTextFiles, sourceMaps, remoteReferences, publicHtmlLeaks, failures, passed: failures.length === 0 }], reportFiles, reportLeakTokens, boundaryChecks, boundaries: { fullCaseFile: "worker-only", momentum: "public projection only", saveSchemaVersion: 1, remoteAnalytics: false, remoteAI: false, hostRewrite: "localhost-only / presentation-only" }, passed: failures.length === 0 && Object.values(boundaryChecks).every(Boolean), qualification: "静态扫描降低意外剧透与远程依赖风险；离线内容仍可被主动反编译，这不是 DRM。" };
writeFileSync(resolve(root, "docs/v1.8-security-scan.json"), `${JSON.stringify(report, null, 2)}\n`, "utf8");
